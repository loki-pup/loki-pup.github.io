...
We also created a placeholder file for the {% raw %}{% url %}{% endraw %}Conf module, named /catalog/{% raw %}{% url %}{% endraw %}s.py. Add the following lines to that file:

```
{% raw %}{% url %}{% endraw %}patterns = [
...
* A {% raw %}{% url %}{% endraw %} pattern, which is an empty string: ''. We'll discuss {% raw %}{% url %}{% endraw %} patterns in detail when working on the other views.

* A view function that will be called if the {% raw %}{% url %}{% endraw %} pattern is detected: views.index, which is the function named index() in the views.py file.

The path() function also specifies a name parameter, which is a unique identifier for this particular {% raw %}{% url %}{% endraw %} mapping. You can use the name to "reverse" the mapper, i.e. to dynamically create a {% raw %}{% url %}{% endraw %} that points to the resource that the mapper is designed to handle. For example, we can use the name parameter to link to our home page from any other page by adding the following link in a template:

```
<a href="{% {% raw %}{% url %}{% endraw %} 'index' %}">Home</a>.
```
> Note: We can hard code the link as in `<a href="/catalog/">Home</a>)`, but if we change the pattern for our home page, for example, to `/catalog/index)` the templates will no longer link correctly. Using a reversed {% raw %}{% url %}{% endraw %} mapping is more robust.
...
The following code snippet is a sample base template from a base_generic.html file. We'll be creating the template for LocalLibrary shortly. The sample below includes common HTML with sections for a title, a sidebar, and main contents marked with the named {% raw %}{% block %}{% endraw %} and end{% raw %}{% block %}{% endraw %} template tags. You can leave the {% raw %}{% block %}{% endraw %}s empty, or include default content to use when rendering pages derived from the template.
...
<html lang="en">
  <head>
    {% {% raw %}{% block %}{% endraw %} title %}
      <title>Local Library</title>
    {% end{% raw %}{% block %}{% endraw %} %}
  </head>
  <body>
    {% {% raw %}{% block %}{% endraw %} sidebar %}
      <!-- insert default navigation text for every page -->
    {% end{% raw %}{% block %}{% endraw %} %}
    {% {% raw %}{% block %}{% endraw %} content %}
...
When defining a template for a particular view, we first specify the base template using the extends template tag — see the code sample below. Then we declare what sections from the template we want to replace (if any), using {% raw %}{% block %}{% endraw %}/end{% raw %}{% block %}{% endraw %} sections as in the base template.

For example, the code snippet below shows how to use the extends template tag and override the content {% raw %}{% block %}{% endraw %}. The generated HTML will include the code and structure defined in the base template, including the default content you defined in the title {% raw %}{% block %}{% endraw %}, but the new content {% raw %}{% block %}{% endraw %} in place of the default one.
...
We will use the following code snippet as the base template for the LocalLibrary website. As you can see, it contains some HTML code and defines {% raw %}{% block %}{% endraw %}s for title, sidebar, and content. We have a default title and a default sidebar with links to lists of all books and authors, both enclosed in {% raw %}{% block %}{% endraw %}s to be easily changed in the future.

Note: We also introduce two additional template tags: {% raw %}{% url %}{% endraw %} and load static. These tags will be explained in following sections.
        ...
        <div class="col-sm-2">
          {% {% raw %}{% block %}{% endraw %} sidebar %}
            <ul class="sidebar-nav">
              <li><a href="{% {% raw %}{% url %}{% endraw %} 'index' %}">Home</a></li>
              <li><a href="">All books</a></li>
              <li><a href="">All authors</a></li>
            </ul>
          {% end{% raw %}{% block %}{% endraw %} %}
        </div>
        <div class="col-sm-10 ">{% {% raw %}{% block %}{% endraw %} content %}{% end{% raw %}{% block %}{% endraw %} %}</div>
      </div>
    </div>
  </body>
</html>
...
Create a new HTML file index.html in /django-locallibrary-tutorial/catalog/templates/ and paste the following code in the file. This code extends our base template on the first line, and then replaces the default content {% raw %}{% block %}{% endraw %} for the template.
...
Your project is likely to use static resources, including JavaScript, CSS, and images. Because the location of these files might not be known (or might change), Django allows you to specify the location in your templates relative to the STATIC_{% raw %}{% url %}{% endraw %} global setting. The default skeleton website sets the value of STATIC_{% raw %}{% url %}{% endraw %} to '/static/', but you might choose to host these on a content delivery network or elsewhere.

Within the template you first call the load template tag specifying "static" to add the template library, as shown in the code sample below. You can then use the static template tag and specify the relative {% raw %}{% url %}{% endraw %} to the required file.
...
<img
  ...
  ```
#### Link to {% raw %}{% url %}{% endraw %}s

The base template above introduced the {% raw %}{% url %}{% endraw %} template tag.

```
<li><a href="{% {% raw %}{% url %}{% endraw %} 'index' %}">Home</a></li>
This tag accepts the name of a path() function called in your {% raw %}{% url %}{% endraw %}s.py and the values for any arguments that the associated view will receive from that function, and returns a {% raw %}{% url %}{% endraw %} that you can use to link to the resource.
```
...
The book list page will display a list of all the available book records in the page, accessed using the {% raw %}{% url %}{% endraw %}: catalog/books/. The page will display a title and author for each record, with the title being a hyperlink to the associated book detail page. The page will have the same structure and navigation as all other pages in the site, and we can, therefore, extend the base template (base_generic.html) we created in the previous tutorial.

#### {% raw %}{% url %}{% endraw %} mapping

Open /catalog/{% raw %}{% url %}{% endraw %}s.py and copy in the line setting the path for 'books/', as shown below. Just as for the index page, this path() function defines a pattern to match against the {% raw %}{% url %}{% endraw %} ('books/'), a view function that will be called if the {% raw %}{% url %}{% endraw %} matches (views.BookListView.as_view()), and a name for this particular mapping.
...
As discussed in the previous tutorial the {% raw %}{% url %}{% endraw %} must already have matched /catalog, so the view will actually be called for the {% raw %}{% url %}{% endraw %}: /catalog/books/.
...
Templates for generic views are just like any other templates (although of course the context/information passed to the template may differ). As with our index template, we extend our base template in the first line and then replace the {% raw %}{% block %}{% endraw %} named content.
...
{% {% raw %}{% block %}{% endraw %} content %}
  ...
  {% if book_list %}
    <ul>
      ...
      <li>
        <a href="{{ book.get_absolute_{% raw %}{% url %}{% endraw %} }}">{{ book.title }}</a>
        ({{book.author}})
      </li>
      {% endfor %}
    </ul>
...
Open the base template (/django-locallibrary-tutorial/catalog/templates/base_generic.html) and insert {% {% raw %}{% url %}{% endraw %} 'books' %} into the {% raw %}{% url %}{% endraw %} link for All books, as shown below. This will enable the link in all pages (we can successfully put this in place now that we've created the "books" {% raw %}{% url %}{% endraw %} mapper).

```
<li><a href="{% {% raw %}{% url %}{% endraw %} 'index' %}">Home</a></li>
<li><a href="{% {% raw %}{% url %}{% endraw %} 'books' %}">All books</a></li>
<li><a href="">All authors</a></li>
```

What does it look like?

You won't be able to build the book list yet, because we're still missing a dependency — the {% raw %}{% url %}{% endraw %} map for the book detail pages, which is needed to create hyperlinks to individual books. We'll show both list and detail views after the next section.

### Book detail page

The book detail page will display information about a specific book, accessed using the {% raw %}{% url %}{% endraw %} catalog/book/<id> (where <id> is the primary key for the book). In addition to fields in the Book model (author, summary, ISBN, language, and genre), we'll also list the details of the available copies (BookInstances) including the status, expected return date, imprint, and id. This will allow our readers to not only learn about the book, but also to confirm whether/when it is available.

#### {% raw %}{% url %}{% endraw %} mapping
Open /catalog/{% raw %}{% url %}{% endraw %}s.py and add the path named 'book-detail' shown below. This path() function defines a pattern, associated generic class-based detail view, and a name.
...
For the book-detail path the {% raw %}{% url %}{% endraw %} pattern uses a special syntax to capture the specific id of the book that we want to see. The syntax is very simple: angle brackets define the part of the {% raw %}{% url %}{% endraw %} to be captured, enclosing the name of the variable that the view can use to access the captured data. For example, <something>, will capture the marked pattern and pass the value to the view as a variable "something". You can optionally precede the variable name with a converter specification that defines the type of data (int, str, slug, uuid, path).
...
#### Passing additional options in your {% raw %}{% url %}{% endraw %} maps
...
For example, given the path shown below, for a request to /my{% raw %}{% url %}{% endraw %}/halibut/ Django will call views.my_view(request, fish='halibut', my_template_name='some_path').

```
path('my{% raw %}{% url %}{% endraw %}/<fish>', views.my_view, {'my_template_name': 'some_path'}, name='a{% raw %}{% url %}{% endraw %}'),
```
...
> Note: The author link in the template above has an empty {% raw %}{% url %}{% endraw %} because we've not yet created an author detail page to link to. Once the detail page exists we can get its {% raw %}{% url %}{% endraw %} with either of these two approaches:

* Use the {% raw %}{% url %}{% endraw %} template tag to reverse the 'author-detail' {% raw %}{% url %}{% endraw %} (defined in the {% raw %}{% url %}{% endraw %} mapper), passing it the author instance for the book:

```
<a href="{% {% raw %}{% url %}{% endraw %} 'author-detail' book.author.pk %}">{{ book.author }}</a>
```

* Call the author model's get_absolute_{% raw %}{% url %}{% endraw %}() method (this performs the same reversing operation):

```
<a href="{{ book.author.get_absolute_{% raw %}{% url %}{% endraw %} }}">{{ book.author }}</a>
```

While both methods effectively do the same thing, get_absolute_{% raw %}{% url %}{% endraw %}() is preferred because it helps you write more consistent and maintainable code (any changes only need to be done in one place: the author model).
...
class Author(models.Model):
    ...
    def get_absolute_{% raw %}{% url %}{% endraw %}(self):
...
With this addition, as soon as you have more than 10 records the view will start paginating the data it sends to the template. The different pages are accessed using GET parameters — to access page 2 you would use the {% raw %}{% url %}{% endraw %} /catalog/books/?page=2.
...
Open /django-locallibrary-tutorial/catalog/templates/base_generic.html and find the "content {% raw %}{% block %}{% endraw %}" (as shown below).

```
{% {% raw %}{% block %}{% endraw %} content %}{% end{% raw %}{% block %}{% endraw %} %}
```

Copy in the following pagination {% raw %}{% block %}{% endraw %} immediately following the {% end{% raw %}{% block %}{% endraw %} %}. The code first checks if pagination is enabled on the current page. If so, it adds next and previous links as appropriate (and the current page number).

```
{% {% raw %}{% block %}{% endraw %} pagination %}
    {% if is_paginated %}
...
We use {{ request.path }} to get the current page {% raw %}{% url %}{% endraw %} for creating the pagination links. This is useful because it is independent of the object that we're paginating.
...
Add the line shown at the bottom of the following {% raw %}{% block %}{% endraw %} to your main HTML template (/django-locallibrary-tutorial/catalog/templates/index.html) at the bottom of the "Dynamic content" section to display the num_visits context variable.
...
Django provides almost everything you need to create authentication pages to handle login, log out, and password management "out of the box". This includes a {% raw %}{% url %}{% endraw %} mapper, views and forms, but it does not include the templates — we have to create our own!

In this section, we show how to integrate the default system into the LocalLibrary website and create the templates. We'll put them in the main project {% raw %}{% url %}{% endraw %}s.
...
> Note: In this case, we could reasonably put the authentication pages, including the {% raw %}{% url %}{% endraw %}s and templates, inside our catalog application. However, if we had multiple applications it would be better to separate out this shared login behavior and have it available across the whole site, so that is what we've shown here!

#### Project {% raw %}{% url %}{% endraw %}s

Add the following to the bottom of the project {% raw %}{% url %}{% endraw %}s.py file (django-locallibrary-tutorial/locallibrary/{% raw %}{% url %}{% endraw %}s.py) file:

```
# Add Django site authentication {% raw %}{% url %}{% endraw %}s (for login, logout, password management)

{% raw %}{% url %}{% endraw %}patterns += [
    path('accounts/', include('django.contrib.auth.{% raw %}{% url %}{% endraw %}s')),
]
```

Navigate to the http://127.0.0.1:8000/accounts/ {% raw %}{% url %}{% endraw %} (note the trailing forward slash!). Django will show an error that it could not find a mapping for this {% raw %}{% url %}{% endraw %}, and list all the {% raw %}{% url %}{% endraw %}s that it tried. From this you can see the {% raw %}{% url %}{% endraw %}s that will work once we have created templates.

> Note: Adding the accounts/ path as shown above adds the following {% raw %}{% url %}{% endraw %}s, along with names (given in square brackets) that can be used to reverse the {% raw %}{% url %}{% endraw %} mappings. You don't have to implement anything else — the above {% raw %}{% url %}{% endraw %} mapping automatically maps the below mentioned {% raw %}{% url %}{% endraw %}s.
...
Now try to navigate to the login {% raw %}{% url %}{% endraw %} (http://127.0.0.1:8000/accounts/login/). This will fail again, but with an error that tells you that we're missing the required template (registration/login.html) on the template search path. You'll see the following lines listed in the yellow section at the top:
...
The {% raw %}{% url %}{% endraw %}s (and implicitly, views) that we just added expect to find their associated templates in a directory /registration/ somewhere in the templates search path.
...
{% {% raw %}{% block %}{% endraw %} content %}
  ...
  <form method="post" action="{% {% raw %}{% url %}{% endraw %} 'login' %}">
    {% csrf_token %}
    <table>
      <tr>
  ...
  {# Assumes you set up the password_reset view in your {% raw %}{% url %}{% endraw %}conf #}
  <p><a href="{% {% raw %}{% url %}{% endraw %} 'password_reset' %}">Lost password?</a></p>

{% end{% raw %}{% block %}{% endraw %} %}
```

This template shares some similarities with the ones we've seen before — it extends our base template and overrides the content {% raw %}{% block %}{% endraw %}. The rest of the code is fairly standard form handling code, which we will discuss in a later tutorial. All you need to know for now is that this will display a form in which you can enter your username and password, and that if you enter invalid values you will be prompted to enter correct values when the page refreshes.
...
If you navigate to the logout {% raw %}{% url %}{% endraw %} (http://127.0.0.1:8000/accounts/logout/) then you'll get an error because Django 5 does not allow logout using GET, only POST. We'll add a form you can use to logout in a minute, but first we'll create the page that users are taken to after logging out.
...
{% {% raw %}{% block %}{% endraw %} content %}
  <p>Logged out!</p>
  <a href="{% {% raw %}{% url %}{% endraw %} 'login'%}">Click here to login again.</a>
{% end{% raw %}{% block %}{% endraw %} %}
```
